...
async function main() {
  ...
  const validPrivacyLevels = ['PRIVATE', 'MINIONS_ONLY', 'MINIONS_AND_FRENEMIES', 'PUBLIC'];
      ...
      try {
        await prisma.playerGame.create({
          data: {
            ...
            isActive: true,
            privacyLevel: validPrivacyLevels[Math.floor(Math.random() * validPrivacyLevels.length)],
      ...
      try {
        ...
        await prisma.clueFinding.create({
          data: {
            ...
            points: points
            // Note: removed isVerified as it's not in the schema
          }
        });
        totalPoints += points;
      } catch (error) {
